...
mkdir -p "${projectdir}/tmp" 2> /dev/null # To be used by temporary files in Drupal
mkdir -p "${projectdir}/drupal/config/default/sync" 2> /dev/null # Config files for Drupal 8 (not accessible by the browser!)
mkdir -p "${projectdir}/drupal/private/default/files" 2> /dev/null # private files go here (not accessible by the browser!)
